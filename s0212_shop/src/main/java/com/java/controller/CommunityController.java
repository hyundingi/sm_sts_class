...
public class CommunityController {
	...
	@ResponseBody // 데이터로 전송 @RestController 랑 같다  [리스폰바디 + 포스트맵핑 조합]
	@PostMapping("/community/api_data")
	public String api_data() throws Exception {
		String service_key = "kmJO3XnaQs4GmkQDSVMNyD8tTTFmVY%2BbVMw5HUx84oes6GVcl8qeAokT7wLBY10HAVnhEnixTSDiV%2B6Ql3Yq3Q%3D%3D";
	...
	@ResponseBody
	@PostMapping("/community/comment/api_data")
	public String comment_api() throws Exception {
		...
		urlBuilder.append("&" + URLEncoder.encode("from", "UTF-8") + "="
				+ URLEncoder.encode("1982-01-01", "UTF-8")); /* 정렬구분 : A=촬영일, B=제목, C=수정일 */
		urlBuilder.append("&" + URLEncoder.encode("until", "UTF-8") + "="
				+ URLEncoder.encode("1982-01-01", "UTF-8")); /* 정렬구분 : A=촬영일, B=제목, C=수정일 */
